...
var _ = Describe("E2ETestSpecificationReconciler", func() {
	...
	// It("should do nothing if no E2ETestRun resource exists", func() {
	//	env := e2eTestRunTestSetup("testdata", "test-01")
	//
	//	Expect(env.Client(platformCluster).DeleteAllOf(env.Ctx, &v1alpha1.E2ETestRun{})).To(Succeed())
	//
	//	testRun := &v1alpha1.E2ETestRun{}
	...
	// })
	//
	// It("should reconcile and create project", func() {
	//	env := e2eTestRunTestSetup("testdata", "test-02")
	//
	//	testRun := &v1alpha1.E2ETestRun{}
	//	Expect(env.Client(platformCluster).Get(env.Ctx, client.ObjectKey{Name: "test-run-02", Namespace: "test-run-02-ns"}, testRun)).To(Succeed())
	//
	...
	//		Expect(testCase.Status).To(Equal("Passed"))
	//	}
	//	projects := &v1alpha2.ProjectList{}
	//	Expect(env.Client(onboardingCluster).List(env.Ctx, projects)).To(Succeed())
	//	Expect(projects.Items).To(BeEmpty())
	// })
	//
	// It("should reconcile and create project and workspace", func() {
	//	env := e2eTestRunTestSetup("testdata", "test-03")
	//
	//	testRun := &v1alpha1.E2ETestRun{}
	//	Expect(env.Client(platformCluster).Get(env.Ctx, client.ObjectKey{Name: "test-run-03", Namespace: "test-run-03-ns"}, testRun)).To(Succeed())
	//
	...
	//	// Verify the test passed
	//	Expect(env.Client(platformCluster).Get(env.Ctx, client.ObjectKey{Name: "test-run-03", Namespace: "test-run-03-ns"}, testRun)).To(Succeed())
	//	Expect(testRun.Status.TestCases).To(HaveLen(2))
	//	for _, testCase := range testRun.Status.TestCases {
	//		Expect(testCase.Status).To(Equal("Passed"))
	//	}
	//	workspaces := &v1alpha2.WorkspaceList{}
	//	Expect(env.Client(onboardingCluster).List(env.Ctx, workspaces)).To(Succeed())
	//	Expect(workspaces.Items).To(BeEmpty())
	//	projects := &v1alpha2.ProjectList{}
	//	Expect(env.Client(onboardingCluster).List(env.Ctx, projects)).To(Succeed())
	//	Expect(projects.Items).To(BeEmpty())
	// })
	//
	// It("should reconcile and create project, workspace and managedcontrolplanev2", func() {
	//	env := e2eTestRunTestSetup("testdata", "test-04")
	//
	//	testRun := &v1alpha1.E2ETestRun{}
	//	Expect(env.Client(platformCluster).Get(env.Ctx, client.ObjectKey{Name: "test-run-04", Namespace: "test-run-04-ns"}, testRun)).To(Succeed())
	//
	...
	//	// Verify the test passed
	//	Expect(env.Client(platformCluster).Get(env.Ctx, client.ObjectKey{Name: "test-run-04", Namespace: "test-run-04-ns"}, testRun)).To(Succeed())
	//	Expect(testRun.Status.TestCases).To(HaveLen(3))
	//	for _, testCase := range testRun.Status.TestCases {
	//		Expect(testCase.Status).To(Equal("Passed"))
	//	}
	//	mcpv2 := &v2alpha1.ManagedControlPlaneV2List{}
	//	Expect(env.Client(onboardingCluster).List(env.Ctx, mcpv2)).To(Succeed())
	//	Expect(mcpv2.Items).To(BeEmpty())
	//	workspaces := &v1alpha2.WorkspaceList{}
	//	Expect(env.Client(onboardingCluster).List(env.Ctx, workspaces)).To(Succeed())
	//	Expect(workspaces.Items).To(BeEmpty())
	//	projects := &v1alpha2.ProjectList{}
